...
def optimal_value(n_states, n_actions, transition_probabilities, reward,
                  discount, threshold=1e-2,VERBOSE=False):
    """
    ...
    -> Array of values for each state
    """
    if VERBOSE: print('value_iteration...')
    v = np.zeros(n_states)
    ...
    while diff > threshold:
        ...
        for s in range(n_states):
            max_v = float("-inf")
            #print(f'value itteration n_actions: {n_actions}')
            for a in range(n_actions):
                tp = transition_probabilities[s, a, :]
                #print(f'VALUE ITERATION SHAPES {np.shape(tp)},{np.shape(reward)}{np.shape(discount)}{np.shape(v)}')
