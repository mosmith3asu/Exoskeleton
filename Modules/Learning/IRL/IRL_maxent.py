...
from Modules.Learning.IRL import IRL_value_iteration as value_iteration
import sys
...
def progressBar(name, value, endvalue,time_elapsed, bar_length=20, width=2):
    percent = float(value) / endvalue

    if percent>0:
        time_per_percent = time_elapsed / (percent*100)
        time_remaining = s2hms(int(time_per_percent*(100-percent*100)))
        #estimated_time = f'|\t Time remain: {int(time_remaining)}s\t'
        estimated_time = f'|\t Time remain: {time_remaining}\t'
    else:  estimated_time = f'|\t Time remain:...\t'
...
def irl(feature_matrix, n_actions, discount, transition_probability,
    ...
    for i in range(epochs):
        ...
        progressBar('Progress',i,epochs,time_elapsed.seconds)
...
def find_feature_expectations(feature_matrix, trajectories,VERBOSE=False):
    """
    ...
    if VERBOSE: print('Trajectory shape', np.shape(trajectories))
    for trajectory in trajectories:
        if VERBOSE:print('Trajectory shape', np.shape(trajectory))
        if VERBOSE: print('Max state idx', trajectory[:,0].max())
        #for state, _, _ in trajectory:
...
def find_expected_svf(n_states, r, n_actions, discount,
                      transition_probability, trajectories,VERBOSE=False):
    """
    ...
    if VERBOSE: print('Maxent.find_expected_svf...')

    if VERBOSE: print(f'|\t n_trajectories={n_trajectories}')
    if VERBOSE:print(f'\t Trajectory length={trajectory_length}')
    if VERBOSE: i = 0
    ...
    for trajectory in trajectories:
        if VERBOSE:
            print(f'|\t Trajectory{i}={int(trajectory[0,0])}')
            i+=1
