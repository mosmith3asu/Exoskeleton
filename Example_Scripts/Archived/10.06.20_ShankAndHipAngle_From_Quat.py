...
def gen_knee_angles(data, show_uninit=False, show_init=True, init_pt=0,
    ...
    for i in range(len(data)):
        ...
        if show_init:
            ...
            angle = angBetweenV(v1, vec)  # + start_displace
            hip_angle.append(angle)

            angle = angBetweenV(v2, vec)
            shank_angle.append(angle)

            angle = angBetweenV(v1, v2)
            knee_angle.append(angle)

        else:
            hip_angle = np.arange(0, len(data))

    return shank_angle, hip_angle, knee_angle
...
if __name__ == "__main__":
    ...
    PLOT_MOCAP = False              # Include motion capture plat
    ...
    # Import Data
    mocap_path = "C:\\Users\\mason\\Desktop\\Thesis\\Patient_files\\SelfCollectedData\\Mocap_MocapTrial.csv"
    imu_path = "C:\\Users\\mason\\Desktop\\Thesis\\Patient_files\\SelfCollectedData\\IMU_MocapTrial.csv"
    shoe_path = "C:\\Users\\mason\\Desktop\\Patient_files\\Patient3S3_shoe.csv"

    data_paths = np.array([  # [{data path}, {initialization/reference point},{IMUs inverted}]
        # ["C:\\Users\\mason\\Desktop\\Patient_files\\Personal1.csv",150,False],
        # ["C:\\Users\\mason\\Desktop\\Patient_files\\Personal2.csv",271,False],
         #["C:\\Users\\mason\\Desktop\\Patient_files\\Personal3_180.csv", 150, True],
        #["C:\\Users\\mason\\Desktop\\Patient_files\\Patient3S1.csv", 348, True],
       # ["C:\\Users\\mason\\Desktop\\Patient_files\\Patient3S2.csv", 27282, True],
        #["C:\\Users\\mason\\Desktop\\Thesis\\Patient_files\\Old\\Patient3S3.csv", 459, True]
        ['C:\\Users\\mason\\Desktop\\Thesis\\Patient_files\\RawData\\IMU_P4S1.csv', 1500, True]
    ])
    ...
    if PLOT_SHOE:
        ...
        # calc knee angles
        knee_angles, hip_angle = gen_knee_angles(data, show_uninit=True, init_pt=11035,
                                                        start_displace=0,
                                                        is_inverted=True)
        ...
        for force1, force2, force3, force4 in data[:, 0:4]:
            if force2 >= 210 or force3 >= 210 or force4 >= 155:  # If shoe is touching
                # shoe_touching.append(max(hip_angle))
    ...
    if PLOT_MOCAP:
        ...
        data = data[(2254 - 1115):, :]
        print(len(data))
        shank_angle, hip_angle, knee_angle = gen_knee_angles(data, show_uninit=True, init_pt=1957,  # 1660,
                                                        start_displace=8)

        if filtered:
            hip_angle = Data_Filters.lfilter(hip_angle, n_iter)
            shank_angle = Data_Filters.lfilter(shank_angle, n_iter)
            knee_angle = Data_Filters.lfilter(knee_angle, n_iter)

        step = 9420. / 9621.
        #step = 1
        print(f'Scaled Step = {step}')
        scaled_t = np.arange(0, 9420, step=step)
        #ax.plot(scaled_t, shank_angle)
        #ax.plot(scaled_t, hip_angle)
        ax.plot(scaled_t,knee_angle)
        ...
        if filtered:
            #ax.legend(["IMU Common frame", "IMU Filtered", "Mocap Converted"])
            ax.legend(["IMU","Motion Capture"])

        else:
            ax.legend(["IMU Raw", "IMU Common Frame", "Mocap Converted"])

        name = "IMU vs Motion Capture (Knee Displacement Validation)"
        ax.set(ylabel='Knee Displacment',xlabel='Timestep t', title=name)
        ax.grid()
    ...
    for path, init_pt, is_inverted in data_paths:
        ...
        init_pt = int(init_pt)
        if is_inverted == "True": is_inverted = True
        else:is_inverted = False
        ...
        # calc knee angles
        shank_angle, hip_angle = gen_knee_angles(data, show_uninit=True, init_pt=init_pt,
        ...
        ax.legend(["Shank", "Hip"])
        name = path.split("\\")[-1]
        ax.set(ylabel='Displacment Anlge', title=name)
        ax.grid()
        plt.tick_params(
    ...
    if export_lastfiles_kneeAng:
        savetxt(date+'_hipAngles_'+ name, hip_angle, delimiter=',')
        savetxt(date + '_shankAngles_' + name, shank_angle, delimiter=',')
        print("exported csv")
